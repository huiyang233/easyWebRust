...
use crate::model::result::{Http, ResultError, WebResult, WebResultPage};
use crate::model::user::UserVo;
...
#[handler]
pub async fn get_user_details(req: &mut Request,depot: &mut Depot) -> Result<WebResult<UserVo>,ResultError> {
    depot.check_permission(&["user"]).await?;
    UserService::get_user_details(req).await
}

#[handler]
pub async fn get_user_by_page(req: &mut Request,depot: &mut Depot) -> Result<WebResult<WebResultPage<UserVo>>, ResultError>{
    depot.check_permission(&["user"]).await?;

...
#[handler]
pub async fn get_self_permission(req: &mut Request,depot: &mut Depot) -> Http<Vec<SysPermissionVo>>{
...
#[handler]
pub async fn delete_sys_permission_by_id(req: &mut Request,depot: &mut Depot) -> Http<String> {
    depot.check_permission(&["permission","permission::delete"]).await?;
    SysPermissionService::delete_sys_permission_by_id(req).await
}

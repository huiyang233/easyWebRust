<template>
  <CommonPage>
    ...
    <AppCard bordered bg="#fafafc dark:black" class="mb-30 min-h-60 rounded-4">
      <n-form class="flex justify-between p-16" @submit.prevent="handleSearch()">
        <n-space wrap :size="[32, 16]">
          <MeQueryItem label="颜色名称" :label-width="100">
            <n-input type="text" v-model:value="queryItems.name" placeholder="请输入颜色名称" clearable />
          </MeQueryItem>
    ...
    <x-n-data-table :loading="loading" :pagination="pagination" :data="tableData" :scroll-x="800"  @update:page="onPageChange">
      ...
      <x-n-data-table-column width="150" fixed="left"  key="name" title="颜色名称" >
      </x-n-data-table-column>
      <x-n-data-table-column width="150" fixed="left"  key="color" title="颜色" >
      </x-n-data-table-column>
    ...
    >
      <n-card :closable="true" @close="close()">
        ...
        <n-form  ref="modalFormRef" label-placement="left" label-align="left" :label-width="100" :model="modal.form">
          ...
          <n-form-item label="颜色名称" path="name" :rule="{
          required: true,
          message: '请输入颜色名称',
          trigger: ['input', 'blur'],
          }">
          ...
          <n-form-item label="颜色" path="color" :rule="{
          required: true,
          message: '请输入颜色',
          trigger: ['input', 'blur'],
          }">
            <n-input v-model:value="modal.form.color" />
          </n-form-item>
...
defineOptions({ name: 'MaterialColor' })

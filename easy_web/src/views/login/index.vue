      ...
      <div class="w-320 flex-col px-20 py-32">
        ...
        <div class="mt-20 flex items-center">
          ...
          <img
            ...
            alt="验证码"
            height="40"
            class="ml-12 w-80 h-34 cursor-pointer rd-5"
            @click="initCaptcha"
          />
        </div>
...
const initCaptcha = throttle(async() => {
  const {flag,data} = await api.getVerificationCode()
  console.log(flag,data)
  if (!flag){
    $message.error('验证码获取失败', { key: 'getVerificationCode' })
    return
  }
  const {uuid,img} = data
...
async function handleLogin() {
  ...
  try {
    ...
    const { data } = await api.login({ username, password: password.toString(),verificationCodeUuid:captchaUUID.value, verificationCode:captcha, loginType:1})
    if (isRemember.value) {
...
async function onLoginSuccess(data = {}) {
  ...
  try {
    await initUserAndPermissions(data)
